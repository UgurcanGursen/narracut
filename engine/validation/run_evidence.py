...
_CHECKS = frozenset({"render_path", "artifact_lifecycle", "storage_pressure", "domain_contract", "source_outcome", "source_audio_safety", "failure_provenance"})
_KIND_BY_CATEGORY = {"render": "render_receipt", "artifact": "artifact_registry", "storage": "storage_admission", "domain": "domain_snapshot"}
_KIND_BY_CHECK = {**_KIND_BY_CATEGORY, "render_path": "render_receipt", "artifact_lifecycle": "artifact_registry", "storage_pressure": "storage_admission", "domain_contract": "domain_snapshot", "source_outcome": "source_capture", "source_audio_safety": "source_audio_direction", "failure_provenance": "failure_code"}
...
def _reference(value: Mapping[str, object], *, run_id: str) -> EvidenceReference:
    ...
    if (item.schema_version != REFERENCE_V1 or item.kind not in {"render_receipt", "artifact_registry", "storage_admission", "domain_snapshot", "source_capture", "source_audio_direction", "failure_code"}
