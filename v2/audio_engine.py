...
from .number_normalizer import CURRENCIES, canonicalize_numbers

def _is_numeric_match(c_val: str, w_val: str) -> bool:
    try:
        return Decimal(c_val) == Decimal(w_val)
    except (InvalidOperation, ValueError):
        return False


_CURRENCY_CODES = frozenset(CURRENCIES.values())


def _parse_canonical_numeric_token(token: str):
    """Return a canonical numeric token's qualifier and value, if present."""
    if not token.startswith("<NUM:") or not token.endswith(">"):
        return None

    parts = token[5:-1].split(":")
    if len(parts) == 1:
        qualifier, value = None, parts[0]
    elif len(parts) == 2:
...
def _numeric_tokens_equivalent(c_token: str, w_token: str) -> bool:
    """Compare canonical numeric tokens without erasing explicit units.

    A plain number may anchor the same currency-qualified value, while percent
    tokens and two explicit but different currency codes remain distinct.
    """
    cue_number = _parse_canonical_numeric_token(c_token)
    word_number = _parse_canonical_numeric_token(w_token)
    if not cue_number or not word_number:
        return False

    cue_qualifier, cue_value = cue_number
    word_qualifier, word_value = word_number
    if not _is_numeric_match(cue_value, word_value):
        return False
    if cue_qualifier == word_qualifier:
        return True
    if cue_qualifier is None:
        return word_qualifier in _CURRENCY_CODES
    if word_qualifier is None:
        return cue_qualifier in _CURRENCY_CODES
    return False


def _candidate_filter_token(token: str) -> str:
    """Keep same-value number/currency candidates visible to local alignment."""
    parsed = _parse_canonical_numeric_token(token)
    if not parsed:
        return token

    qualifier, value = parsed
    if qualifier == "PERCENT":
        return f"<NUM:PERCENT:{value}>"
    return f"<NUM:VALUE:{value}>"
...
def resolve_audio_for_block(block, output_path: str, rate_adjustment: str = None, pause_policy: str = "normal") -> bool:
    ...
    if edge_tts:
        try:
            generate_tts_edge_sync(block.narration, output_path, rate_adjustment=rate_adjustment, pause_policy=pause_policy)
            return True
        except Exception:
            pass
...
def generate_tts_edge_sync(text: str, output_path: str, voice: str = "en-US-ChristopherNeural", rate_adjustment: str = None, pause_policy: str = "normal") -> None:
    if not edge_tts:
        raise RuntimeError("edge_tts not installed.")
    
    async def _gen():
        opt_text = text
        if pause_policy == "shorter":
            opt_text = opt_text.replace(". ", " ").replace("? ", " ").replace("! ", " ")
            opt_text = opt_text.replace(", ", " ")
        else:
            opt_text = opt_text.replace(". ", ", ").replace("? ", ", ").replace("! ", ", ")
            opt_text = opt_text.replace(", and ", " and ")
            
        rate = rate_adjustment if rate_adjustment else EDGE_TTS_RATE
        communicate = edge_tts.Communicate(opt_text, voice, rate=rate)
...
def transcribe_audio_aligned(audio_path: str) -> list:
    ...
    import json
    with open("whisper_debug.json", "w") as f:
...
def find_cue_time(words: list, cue_text: str, return_end: bool = False, min_start_time: float = 0.0) -> dict:
    ...
    def _norm_text(w):
        if _parse_canonical_numeric_token(w):
            return w
        return re.sub(r'[^\w\s]', '', w).lower().strip()
        
    cue_norm = canonicalize_numbers(cue_text)
    cue_words = [_norm_text(t["word"]) for t in cue_norm.tokens if _norm_text(t["word"])]
    ...
    norm_words = []
    for t in transcript_norm.tokens:
        nw = _norm_text(t["word"])
        if nw:
            t_copy = t.copy()
            t_copy["word"] = nw
            norm_words.append(t_copy)
    
    import json
    with open("norm_words_debug.json", "w") as f:
        json.dump([w["word"] for w in norm_words], f, indent=2)
    ...
    candidate_cue_words = [_candidate_filter_token(word) for word in cue_words]
    ...
    for i in range(len(valid_words)):
        ...
        candidate_window_texts = [_candidate_filter_token(word) for word in window_texts]
        sm = difflib.SequenceMatcher(None, candidate_cue_words, candidate_window_texts)
        ...
        for ci in range(1, m + 1):
            for wj in range(1, n + 1):
                ...
                if c_clean == w_clean:
                    is_match = True
                elif c_clean.rstrip("s") == w_clean.rstrip("s"):
                    is_match = True
                elif _numeric_tokens_equivalent(c_word, w_word):
                    is_match = True
                elif c_clean in {"was", "is", "were", "are", "with", "the", "a", "an", "of", "to", "in", "for", "on", "at", "by"} and w_clean in {"was", "is", "were", "are", "with", "the", "a", "an", "of", "to", "in", "for", "on", "at", "by"}:
                    is_match = True
        ...
        if cue_nums:
            # Check each cue number against the numeric tokens in this window.
            all_matched = True
            for c in cue_nums:
                if not any(_numeric_tokens_equivalent(c, w) for w in matched_nums):
                    all_matched = False
                    break
            
            if not all_matched or len(cue_nums) != len(matched_nums):
    ...
    # Deduplicate candidates based on matched time (allow 2s margin)
...
def generate_pacing_report(words: list, audio_duration: float) -> dict:
    ...
    silence_ratio = total_silence / audio_duration if audio_duration > 0 else 0
    
    status = "valid"
    reasons = []
    
    if not (100 <= gross_wpm <= 150):
        status = "invalid"
        reasons.append("PACING_GROSS_WPM_HIGH" if gross_wpm > 150 else "PACING_GROSS_WPM_LOW")
        
    if not (110 <= active_wpm <= 160):
        status = "invalid"
        reasons.append("PACING_ACTIVE_WPM_HIGH" if active_wpm > 160 else "PACING_ACTIVE_WPM_LOW")
        
    if silence_ratio > 0.40:
        status = "invalid"
        reasons.append("PACING_SILENCE_RATIO_HIGH")
        
    mean_pause = total_silence / len(pauses) if pauses else 0
    if mean_pause > 0.90:
        status = "invalid"
        reasons.append(f"Mean sentence pause too high: {mean_pause:.3f}s (Target: <= 0.90s)")
        
    p95_pause = np.percentile(pauses, 95) if pauses else 0
    if p95_pause > 1.5:
        status = "invalid"
        reasons.append(f"P95 pause too high: {p95_pause:.3f}s (Target: <= 1.5s)")
    ...
    return {
        ...
        "mean_pause": round(mean_pause, 3),
        "longest_pause": round(longest_pause, 2),
        "p95_pause": round(p95_pause, 3),
